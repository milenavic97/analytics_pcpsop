...
const SUGESTOES: Record<string, string[]> = {
  ordens:   ["Resumo geral das OPs", "O que está faltando?", "Quais podem abrir agora?", "OPs em quarentena"],
  overview: ["Como estão as liberações?", "Como está o faturamento?", "Resumo executivo do mês"],
...
export function PCPChat() {
  ...
  async function handleSend(texto?: string) {
    ...
    const userMsg: ChatMessage = {
      id: crypto.randomUUID(), role: "user", text: pergunta, time: nowTime(),
    }
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        text: data.resposta || "Sem resposta.", time: nowTime(),
      }])
    } catch (e: unknown) {
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="rounded-2xl px-4 py-3 text-sm"
                  style={{ background: "#FFFFFF", border: "1px solid var(--border)", color: "var(--text-secondary)" }}>
          ...
          <div className="border-t p-3" style={{ borderColor: "var(--border)", background: "#FFFFFF" }}>
            <div className="flex items-center gap-2 rounded-2xl border p-2"
              ...
              <div className="flex h-10 w-10 items-center justify-center rounded-xl"
                style={{ background: "#EEF2FF", color: "#1B3A5C" }}>
                <Sparkles size={18} />
              </div>
              <input
                value={input}
