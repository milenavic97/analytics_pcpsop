    ...
    <div className="rounded-xl p-4"
      ...
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0 mt-0.5">
          <AlertOctagon size={16} style={{ color: isFalta ? "#DC2626" : "#F59E0B" }} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-xs font-bold uppercase tracking-wider"
              style={{ color: isFalta ? "#991B1B" : "#92400E" }}>
          ...
          <p className="text-sm font-semibold mb-3" style={{ color: isFalta ? "#7F1D1D" : "#78350F" }}>
            {gargalo.descricao}
          </p>
          ...
          <div className="mb-3">
            ...
            <div className="h-2 rounded-full overflow-hidden" style={{ background: isFalta ? "#FECACA" : "#FDE68A" }}>
              <div className="h-full rounded-full transition-all"
                style={{
                  width: `${pctUsado}%`,
                  background: isFalta ? "#DC2626" : "#F59E0B",
                }} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="rounded-lg px-3 py-2" style={{ background: "rgba(255,255,255,0.7)" }}>
              <p className="font-medium mb-0.5" style={{ color: isFalta ? "#991B1B" : "#92400E", opacity: 0.7 }}>Necessário</p>
              <p className="font-bold" style={{ color: isFalta ? "#7F1D1D" : "#78350F" }}>
                {fmt(gargalo.necessario)} <span style={{ fontWeight: 400, opacity: 0.7 }}>{gargalo.unidade}</span>
              </p>
            </div>
            <div className="rounded-lg px-3 py-2" style={{ background: "rgba(255,255,255,0.7)" }}>
              <p className="font-medium mb-0.5" style={{ color: isFalta ? "#991B1B" : "#92400E", opacity: 0.7 }}>
                Chegou{gargalo.saldo_chegou_98 > 0 ? " (arm. 01)" : ""}
              </p>
              <p className="font-bold" style={{ color: gargalo.saldo_chegou > 0 ? "#16A34A" : "#DC2626" }}>
                {fmt(gargalo.saldo_chegou)} <span style={{ fontWeight: 400, opacity: 0.7 }}>{gargalo.unidade}</span>
              </p>
            </div>
            <div className="rounded-lg px-3 py-2" style={{ background: "rgba(255,255,255,0.7)" }}>
              <p className="font-medium mb-0.5" style={{ color: isFalta ? "#991B1B" : "#92400E", opacity: 0.7 }}>Faltante</p>
              <p className="font-bold" style={{ color: isFalta ? "#DC2626" : "#F59E0B" }}>
                {fmt(gargalo.faltante)} <span style={{ fontWeight: 400, opacity: 0.7 }}>{gargalo.unidade}</span>
              </p>
            </div>
          </div>
...
function EditModal({ op, onClose, onSaved, isNova = false }: {
  op: OPEditavel
  onClose: () => void
  onSaved: (atualizado: Partial<OPEditavel>) => void
  isNova?: boolean
}) {
    ...
    <div className="fixed inset-0 z-[999] flex items-end md:items-center justify-center p-0 md:p-4"
      ...
      <div className="w-full md:max-w-2xl rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden"
        ...
        <div className="overflow-y-auto p-5 space-y-5" style={{ maxHeight: "calc(92vh - 140px)" }}>
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Lote</label>
                <input className={inp} style={s} value={form.lote} onChange={e => setForm(f => ({ ...f, lote: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Código</label>
                <input className={inp} style={s} value={form.codigo} onChange={e => setForm(f => ({ ...f, codigo: e.target.value }))} />
              </div>
            </div>
            <div className="flex flex-col gap-1.5 mt-3">
              <label className="card-label">Produto</label>
              <input className={inp} style={s} value={form.produto} onChange={e => setForm(f => ({ ...f, produto: e.target.value }))} />
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <div className="flex flex-col gap-1.5">
                ...
                <select className={inp} style={s} value={form.linha} onChange={e => setForm(f => ({ ...f, linha: e.target.value }))}>
                  <option value="ENVASE_L1">Envase L1</option>
                  <option value="ENVASE_L2">Envase L2</option>
                  <option value="EMBALAGEM">Embalagem</option>
                </select>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Nº OP</label>
                <input className={inp} style={s} value={form.op_numero} placeholder="Ex: 90311"
                  onChange={e => setForm(f => ({ ...f, op_numero: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Quantidade</label>
                <input type="number" className={inp} style={s} value={form.quantidade}
                  onChange={e => setForm(f => ({ ...f, quantidade: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Tempo (h)</label>
                <input type="number" className={inp} style={s} value={form.tempo_horas}
                  onChange={e => setForm(f => ({ ...f, tempo_horas: e.target.value }))} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
              {isEnvase && (
                <>
                  <div className="flex flex-col gap-1.5">
                    <label className="card-label">Lav. Êmb e Lacre</label>
                    <input type="date" className={inp} style={s} value={form.data_lavagem_emb || ""}
                      onChange={e => setForm(f => ({ ...f, data_lavagem_emb: e.target.value }))} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="card-label">Lav. e Pesagem</label>
                    <input type="date" className={inp} style={s} value={form.data_lavagem_pesagem || ""}
                      onChange={e => setForm(f => ({ ...f, data_lavagem_pesagem: e.target.value }))} />
                  </div>
                </>
              )}
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Início Fabricação</label>
                <input type="date" className={inp} style={s} value={form.data_inicio_fabricacao || ""}
                  onChange={e => setForm(f => ({ ...f, data_inicio_fabricacao: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Data Fim</label>
                <input type="date" className={inp} style={s} value={form.data_fim || ""}
                  onChange={e => setForm(f => ({ ...f, data_fim: e.target.value }))} />
              </div>
              {isEnvase && (
                <div className="flex flex-col gap-1.5">
                  <label className="card-label">Término Real</label>
                  <input type="date" className={inp} style={s} value={form.data_termino || ""}
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Observação da planilha</label>
                <input className={inp} style={s} value={form.observacoes}
                  onChange={e => setForm(f => ({ ...f, observacoes: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="card-label">Anotação interna</label>
                <textarea className={inp} style={{ ...s, resize: "vertical", minHeight: 72 }}
                  value={form.anotacao} placeholder="Anotações sobre esta OP..."
        ...
        <div className="flex items-center justify-end gap-3 px-5 py-4" style={{ borderTop: "1px solid var(--border)" }}>
          <button onClick={onClose} disabled={saving} className="rounded-xl border px-4 py-2 text-sm font-semibold"
            style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}>
            Cancelar
          </button>
          <button onClick={handleSave} disabled={saving}
            className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white"
            style={{ background: "var(--bg-sidebar)", opacity: saving ? 0.7 : 1 }}>
      ...
      <div className="w-full md:max-w-lg rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden"
        ...
        <div className="overflow-y-auto p-5 space-y-4" style={{ maxHeight: "calc(85vh - 80px)" }}>
          {LINHAS.map(linha => {
            ...
            return (
              <div key={linha} className="rounded-xl p-4"
                ...
                {tipo !== "total" && total > 0 && (
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
                    <div className="h-full rounded-full" style={{
                      width: `${pct}%`,
                      background: tipo === "com_material" ? "#16A34A" : tipo === "sem_material" ? "#DC2626" : tipo === "abertas" ? "#2563EB" : "#6B7280",
                    }} />
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {topMat.map(([codigo, info]) => (
                  <div key={codigo} className="flex items-center justify-between rounded-lg px-3 py-2"
                    style={{ background: "#FEF2F2", border: "1px solid #FECACA" }}>
                    <div className="min-w-0">
                      <p className="text-xs font-mono" style={{ color: "#991B1B" }}>{codigo}</p>
                      <p className="text-xs truncate" style={{ color: "#7F1D1D" }}>{info.descricao}</p>
                    </div>
                    <span className="text-xs font-bold ml-3 flex-shrink-0" style={{ color: "#DC2626" }}>
                      {info.count} OP{info.count !== 1 ? "s" : ""}
                    </span>
                  </div>
                ))}
              </div>
...
function SummaryCard({ label, value, sub, color, Icon, onClick }: {
  ...
  return (
    <button onClick={onClick} className="card flex flex-col gap-3 p-4 text-left w-full hover:shadow-md transition-all"
        ...
        >
          ...
          <div className="max-h-56 overflow-y-auto p-1">
            <button
              ...
              style={{ color: value === "" ? "var(--bg-sidebar)" : "var(--text-secondary)" }}
            >
  ...
  return (
    <div className="card p-4 md:p-5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7">
        ...
        <SearchableSelect label="Linha" value={linhaSel} options={linhaOptions} placeholder="Todas as linhas" onChange={onLinha} />
        <SearchableSelect label="Status" value={statusSel} options={statusOptions} placeholder="Todos os status" onChange={onStatus} />
        <SearchableSelect label="Tipo" value={tipoSel} options={tipoOptions} placeholder="Todos os tipos" onChange={onTipo} />
        <SearchableSelect label="Lote" value={loteSel} options={loteOptions} placeholder="Todos os lotes" onChange={onLote} />
        <SearchableSelect label="Código" value={codigoSel} options={codigoOptions} placeholder="Todos os códigos" onChange={onCodigo} />
        <SearchableSelect label="Produto" value={produtoSel} options={produtoOptions} placeholder="Todos os produtos" onChange={onProduto} />
      </div>
    </div>
  )
}
...
function OPRow({ op, selecionado, onSelect, onEdit, produtoColWidth, mostrarDivisoria }: {
  ...
  const rowBg = selecionado
    ? "rgba(27,58,92,0.10)"
    : aberto ? cfg.bg + "33" : undefined
  ...
  return (
    <>
      <tr
        ...
        <td className="pl-3 py-2.5 w-8 border-b" style={{ borderColor: GRID_COLOR }}
          onClick={e => e.stopPropagation()}>
        ...
        <Td className="hidden md:table-cell w-20 font-mono text-xs" style={{ color: "var(--text-secondary)" }}>
          {op.codigo}
        </Td>
        <Td className="hidden md:table-cell w-24" style={{ color: "var(--text-primary)" }}>
          {LINHA_LABEL[op.linha] || op.linha}
        </Td>
        ...
        <Td className="hidden lg:table-cell w-24 font-mono text-xs" style={{ color: "var(--text-secondary)" }}>
          {op.op_numero || "—"}
        </Td>
        <Td className="hidden lg:table-cell w-24 font-semibold" style={{ color: "var(--text-primary)" }}>
          {fmt(op.quantidade)}
        </Td>
        <Td className="hidden xl:table-cell w-28 text-xs" style={{ color: "var(--text-secondary)" }}>
          {fmtData((op as OPEditavel).data_lavagem_emb)}
        </Td>
        <Td className="hidden xl:table-cell w-28 text-xs" style={{ color: "var(--text-secondary)" }}>
          {fmtData((op as OPEditavel).data_inicio_fabricacao)}
        </Td>
        <Td className="hidden lg:table-cell w-24 text-xs" style={{ color: "var(--text-secondary)" }}>
          {fmtData(op.data_fim)}
        </Td>

        {/* Status — com tooltip do gargalo se houver */}
        <Td className="w-36">
      ...
      {/* Gargalo da própria OP: fica abaixo da linha da OP crítica */}
      {mostrarDivisoria && (
        <FifoDivider label={
          op.gargalo
            ? `Gargalo da OP: ${op.gargalo.descricao} (${op.gargalo.codigo_comp})`
            : "Gargalo da OP identificado"
        } />
      )}
        ...
        <tr>
          <td colSpan={15} className="px-4 pb-4 pt-1">
            <div className="rounded-xl p-4 space-y-4"
              style={{ background: cfg.bg + "44", border: `1px solid ${cfg.border}` }}>

              {((op as OPEditavel).observacoes || (op as OPEditavel).anotacao) && (
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  {(op as OPEditavel).observacoes && (
                    <div className="rounded-lg px-3 py-2.5"
                      style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
                      <p className="card-label mb-1">Observação</p>
                      <p className="text-sm" style={{ color: "var(--text-primary)" }}>{(op as OPEditavel).observacoes}</p>
                    </div>
                  )}
                  {(op as OPEditavel).anotacao && (
                    <div className="rounded-lg px-3 py-2.5"
                      style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
                      <p className="card-label mb-1">Anotação interna</p>
                      <p className="text-sm" style={{ color: "var(--text-primary)" }}>{(op as OPEditavel).anotacao}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {op.linha !== "EMBALAGEM" && (
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4 text-xs">
                  {[
                    { label: "Lav. Êmb e Lacre", val: (op as OPEditavel).data_lavagem_emb },
                    { label: "Lav. e Pesagem",   val: (op as OPEditavel).data_lavagem_pesagem },
                    { label: "Início Fabricação", val: (op as OPEditavel).data_inicio_fabricacao },
                    { label: "Término Real",      val: (op as OPEditavel).data_termino },
                  ].map(({ label, val }) => (
                      ...
                      <thead>
                        <tr style={{ borderBottom: "1px solid var(--border)" }}>
                          {["Código", "Descrição", "TP", "Necessário", "Saldo Atual", "Saldo Restante", "Saldo 98", "Status"].map(h => (
                            <th key={h} className="pb-2 pr-4 text-left font-semibold uppercase tracking-wider"
                              style={{ color: "var(--text-secondary)", fontSize: 10 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                            ...
                            <tr key={i} style={{ borderBottom: "1px solid var(--border)", background: comp.status !== "ok" ? compCfg.bg + "55" : undefined }}>
                              ...
                              <td className="py-2 pr-4">
                                <span className="rounded px-1.5 py-0.5 font-mono font-bold"
                                  style={{ background: "var(--bg-secondary)", color: "var(--text-secondary)", fontSize: 10 }}>{comp.tp}</span>
                              </td>
                              ...
                              <td className="py-2 pr-4"
                                style={{ color: comp.saldo_98 > 0 ? "#F59E0B" : "var(--text-secondary)", fontWeight: comp.saldo_98 > 0 ? 600 : 400 }}
...
function OPTable({ ops, selecionados, onSelect, onSelectAll, onEdit }: {
  ...
  // Calcula quais OPs devem mostrar o banner de gargalo.
  // Regra: mostrar a primeira OP com gargalo de MP em cada bloco (PA e PI),
  // depois de pelo menos uma OK no mesmo tipo. O banner aparece abaixo da própria OP crítica.
  const divisoriaSet = new Set<number>()
  const controleDivisoria: Record<"PA" | "PI", { viuOk: boolean; mostrou: boolean }> = {
  ...
  for (let i = 0; i < ops.length; i++) {
    ...
    const ctrl = controleDivisoria[tipo]
    if (op.status === "ok") { ctrl.viuOk = true; continue }
    if (!ctrl.mostrou && ctrl.viuOk && (op.status === "falta" || op.status === "quarentena") && op.fifo_posicao != null) {
      divisoriaSet.add(i)
      ctrl.mostrou = true
    }
  }
  ...
  return (
    <div className="rounded-2xl overflow-hidden"
      style={{ border: "1px solid var(--border)", cursor: isResizing ? "col-resize" : undefined }}>
      <div className="overflow-auto" style={{ maxHeight: "60vh" }}>
        <table className="w-full min-w-[900px] border-separate border-spacing-0">
          <thead style={{ position: "sticky", top: 0, zIndex: 10 }}>
            <tr style={{ background: TABLE_HEADER_BG }}>
              <th className="pl-3 py-3 w-8">
                <input type="checkbox" checked={todosSelect}
                  onChange={e => onSelectAll(e.target.checked)}
                  style={{ accentColor: "#fff" }} className="rounded" />
              </th>
              <th className="w-5 py-3" />
              {/* Coluna FIFO */}
              <th className="px-2 py-3 w-8 text-center text-[11px] font-semibold uppercase tracking-wider"
                style={{ color: "rgba(255,255,255,0.65)" }} title="Posição na fila FIFO">#</th>
              <th className={`${thCls} w-28`} style={thStyle}>Lote</th>
              {/* Produto redimensionável */}
...
function exportarExcel(ops: OPEditavel[], mesRef: string) {
  ...
  const rows = ops.map(op => [
    op.fifo_posicao ?? "",
    op.lote,
    op.produto || op.codigo,
    op.codigo,
    LINHA_LABEL[op.linha] || op.linha,
    tipoProduto(op.linha),
    op.op_numero || "",
    op.quantidade,
    fmtData(op.data_lavagem_emb),
    fmtData(op.data_inicio_fabricacao),
    fmtData(op.data_fim),
  ...
  a.href = url
  a.download = `ops_${mesRef || "export"}.csv`
  a.click()
...
export function OrdensPage() {
  ...
  const tipoOptions = useMemo(
    () => uniqueOptions(ops.map(op => tipoProduto(op.linha))),
    [ops]
  )
  const loteOptions = useMemo(
    () => uniqueOptions(ops.map(op => op.lote)),
    [ops]
  )
  const codigoOptions = useMemo(
    () => uniqueOptions(ops.map(op => op.codigo)),
    [ops]
  )
  const produtoOptions = useMemo(
    () => uniqueOptions(ops.map(op => op.produto || op.codigo)),
    [ops]
  )

  const opsFiltradas = ops.filter(op => {
    if (linhaSel && op.linha !== linhaSel) return false
    if (statusSel && op.status !== statusSel) return false
    if (tipoSel && tipoProduto(op.linha) !== tipoSel) return false
    if (loteSel && op.lote !== loteSel) return false
    if (codigoSel && op.codigo !== codigoSel) return false
    if (produtoSel && (op.produto || op.codigo) !== produtoSel) return false
    return true
  })
  ...
  return (
    <div className="min-h-screen space-y-5 p-3 md:space-y-6 md:p-6">
      ...
      <div className="fade-in flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
            Confira quais OPs sem emissão têm material disponível para abertura no Protheus.
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {selecionados.size > 0 && (
            <button
              onClick={() => exportarExcel(opsFiltradas.filter(op => selecionados.has(op.id || `${op.lote}-${op.codigo}`)), mesSel)}
          ...
          <button onClick={() => exportarExcel(opsFiltradas, mesSel)}
            className="flex h-10 items-center gap-2 rounded-xl border px-4 text-xs font-semibold"
            style={{ cursor: "pointer" }}>
            <Download size={14} />
            Exportar tudo
          </button>
          <button onClick={() => setNovaOpModal(true)}
            className="flex h-10 items-center gap-2 rounded-xl px-4 text-xs font-semibold text-white"
            style={{ background: "var(--bg-sidebar)" }}>
            <Plus size={14} />
            Nova OP
          </button>
          <button onClick={buscar} disabled={loading || !mesSel}
            className="flex h-10 items-center gap-2 rounded-xl border px-4 text-xs font-semibold"
      ...
      <FilterPanel
        ...
        codigoOptions={codigoOptions}
        produtoOptions={produtoOptions}
        tipoOptions={tipoOptions}
        onMes={setMesSel}
        onLinha={setLinhaSel}
        onStatus={setStatusSel}
        onTipo={setTipoSel}
        onLote={setLoteSel}
        onCodigo={setCodigoSel}
        onProduto={setProdutoSel}
      />
      ...
      {!loading && !erro && opsFiltradas.length > 0 && (
        <div className="fade-in space-y-2">
          ...
          <OPTable ops={opsFiltradas} selecionados={selecionados}
            onSelect={handleSelect} onSelectAll={handleSelectAll} onEdit={setOpEditando} />
        </div>
      )}

      {!loading && !erro && dados && opsFiltradas.length === 0 && (
        <div className="card p-10 text-center text-sm fade-in" style={{ color: "var(--text-secondary)" }}>
          Nenhuma OP encontrada para os filtros selecionados.
        </div>
      )}

      {!loading && !erro && !dados && !mesSel && (
        <div className="card p-10 text-center text-sm fade-in" style={{ color: "var(--text-secondary)" }}>
          Nenhuma programação carregada. Faça o upload da planilha de OPs na aba Dados.
        </div>
      )}
      ...
      {novaOpModal && (
        <EditModal
          op={{
            id: undefined, mes_ref: mesSel,
            lote: "", produto: "", codigo: "", linha: "ENVASE_L1" as OPResult["linha"],
            op_numero: null, quantidade: 0, status: "ok" as StatusOP,
            alertas: [], detalhes: [], data_fim: null,
          } as OPEditavel}
